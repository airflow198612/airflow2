...
class REST_API(BaseView):
    ...
    def variables(self):
        ...
        set_key = request.args.get('set_key')
        set_val = request.args.get('set_val')
        ...
        if set_key:
          if set_val:
            command_split.extend(["--set", set_key,set_val])
          else:
            return get_400_error_response(base_response, "set_key provided but set_val is not provided")
    ...
    @expose(url_dict.get("CLEAR_URL"))
    @http_token_secure
    def clear(self):
        logging.info("REST_API.clear() called")
        base_response = get_base_response()
        dag_id = request.args.get('dag_id')
        subdir = request.args.get('subdir')
        ...
        if request.args.get("upstream") is not None:
            command_split.append("--upstream")
        if request.args.get("downstream") is not None:
            command_split.append("--downstream")

        command_split.extend(["--no_confirm",dag_id])
