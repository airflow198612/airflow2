...
# Description: TensorFlow spin up and terminate operators
...
class TensorFlowOperator(BaseOperator):
    ...
    def __init__(
        ...
        super(TensorFlowOperator, self).__init__(*args, **kwargs)
    ...
    def get_auto_scaling_group(self):
        ...
        if not asg_found:
            raise AirflowException(
                "Cannot found TensorFlow instance with the type {instance_type}".format(
...
class TensorFlowSpinUpOperator(TensorFlowOperator):
    """
    Spins up TensorFlow node
    """
    ui_color = '#D4EBFF'
    ...
    def __init__(
        ...
        super(TensorFlowSpinUpOperator, self).__init__(task_id='tensorflow_spinup', instance_type=instance_type, *args,
                                                       **kwargs)
        self.instance_type = instance_type.lower()

    def execute(self, context):

        self.xcom_instance_list = context['ti'].xcom_pull(key='instance_list', task_ids='tensorflow_spinup')
        ...
        if self.instance_count == 0:
            self.scale_up_asg()
            logging.info('tty endpoint is')
            endpoint = 'tty-tf-' + cluster + '-tensorflow_' + self.instance_type + '.ngap2.nike.com'
    ...
    def scale_up_asg(self):
        ...
        if not success:
            raise AirflowException("cannot spin up TensorFlow instance")
...
class TensorFlowTerminateOperator(TensorFlowOperator):
    """
    Terminate TensorFlow node
    """
    ui_color = '#D4EBFF'
    ...
    def __init__(
            ...
            instance_type,
            *args, **kwargs):
        super(TensorFlowTerminateOperator, self).__init__(task_id='tensorflow_terminate', instance_type=instance_type,
                                                          *args, **kwargs)
        self.instance_type = instance_type.lower()
    ...
    def execute(self, context):
        ...
        self.client = boto3.client('autoscaling', region_name='us-east-1')
        self.ec2_client = boto3.client('ec2', region_name='us-east-1')
        self.instance_id = context['ti'].xcom_pull(key='instance_id', task_ids='tensorflow_spinup')
        self.xcom_instance_list = context['ti'].xcom_pull(key='instance_list', task_ids='tensorflow_spinup')
        if self.xcom_instance_list is None:
            self.xcom_instance_list = []
        self.get_auto_scaling_group()
    ...
    def del_route53(self):
        ...
        prefix = tty_prefix + '/' + cluster + '/' + 'tensorflow_' + self.instance_type
        endpoint = 'tty-tf-' + cluster + '-tensorflow_' + self.instance_type
    ...
    def scale_down_asg(self):
        ...
        # wait 30 seconds to make sure the log is going to Kibana
        logging.info(
        ...
        if response['ResponseMetadata']['HTTPStatusCode'] != 200:
            logging.info(response)
            raise AirflowException("cannot scale down TensorFlow instance")
...
class TensorFlowPlugin(AirflowPlugin):
    name = "TensorFlowPlugin"
    operators = [
        TensorFlowSpinUpOperator,
        TensorFlowTerminateOperator
    ]
